...
# Konfiguration
Da aktuell die IP-Adresse des Hosts nicht korrekt über den Server ermittelt werden kann, sollte man für das beste Erlebnis diese zuvor manuell in die Datei ./streamlit/pages/settings.py eintragen. (Zeile 24:   ip_address:str = "xxx.yyy.zzz.xyz"   # always set your local IP-Address here! (Windows cmd: ipconfig / Linux shell: ifconfig ) )
...
>`http://<ip-des-servers>:8001/driftapi/game/<Lobby-Name>/<Stage_ID>`
 
wobei du `<ip-des-server>` mit der IP des Computers auf dem du den Server laufen lässt, ersetzt, `<Lobby-Name>` der Name der Lobby ist, in der das Spiel erstellt wurde und `<Stage_ID>` in der Regel 1 ist (außer beim Spielmodus Etappen-Rennen, hier steht `<Stage_ID>` für die Nummer der Etappe). Beispiel: Wenn mein PC die IP 192.168.40.23 hat, der Name der Lobby Lobby1 lautet und ein einfaches Spiel im Modus RACE erstellt wurde, dann muss ich in der DR!FT App beim Feld URL http://192.168.40.23:8001/driftapi/game/Lobby1/1 eingeben. Die game-id in der app muss genau so heißen wie die vom Spiel das du erstellst und dein Nutzername sollte möglichst keine Sonderzeichen wie ' enthalten.
...
# OnlineDB
Du möchtest gerne Online gegen Mitspieler antreten? Kein Problem, mit ein paar Schritten kann die lokale Datenbank (MongoDB) durch eine Online-Variante (MongoDB Atlas - in der richtigen Konfuguration völlig kostenlos) ausgetauscht werden. Das Frontend läuft hierbei nach wie vor lokal auf eurem Host - was bedeutet, dass jeder Mitspieler eine Installation des Dr!ft Racingservers benötigt - die Daten werden jedoch zur online Datenbank kommuniziert, wodurch man den Multiplayer auch über Distanz spielen kann. In einem ersten Schritt muss man sich hierbei einen freien Cluster anlegen. Eine gute Anleitung findet sich unter "How to Set Up MongoDB Cluster in the Cloud for Free" (siehe: https://www.makeuseof.com/mongodb-cluster-cloud-free-setup/ ). Wichtig hierbei ist, dass bei Schritt 3 und 4 als Driver Python gewählt wird. Ob Version 3.6 or later, 3.11 or later bzw. 3.12 or later spielt eigentlich keine Rolle, da der angezeigte Code stets der gleiche ist. Der angezeigte Code (Beispiel: `mongodb+srv://drift_0815:<password>@cluster0.ua0cw.mongodb.net/?retryWrites=true&w=majority` muss für den Drift Racingserver kopiert werden. Folgende Stellen im Code müssen für einen Online-Betrieb geändert werden: In der Datei ./.env muss der Eintrag `DRIFTAPI_DATABASE_URL=mongodb://driftapi-db:27017` ausgetauscht werden durch den kopierten Code, also z.B. `DRIFTAPI_DATABASE_URL=mongodb+srv://drift_0815:<password>@cluster0.ua0cw.mongodb.net/?retryWrites=true&w=majority` und zudem muss die nächste Zeile `DRIFTAPI_DATABASE_NAME=driftapi-test-db` ersetzt werden durch den User Namen, also z.B. `DRIFTAPI_DATABASE_NAME=drift_0815` . In einem nächsten Schritt müssen in der Datei ./docker-compose.yml alle Einträge zur lokalen Datenbank entfernt werden. Dies beinhaltet sowohl die Zeilen `depends_on: - driftapi-db` als auch den kompletten letzten Eintrag zu `driftapi-db: ...` . Zu guter Letzt müssen in der Datei ./app/server/database.py noch kleine Änderungen vollzogen werden. Hierbei setzt man über die Zeile `client = motor.motor_asyncio.AsyncIOMotorClient(settings.database_url)` den Eintrag (mit der kopierten Adresse) `MONGO_DETAILS = "mongodb+srv://drift_0815:<password>@cluster0.ua0cw.mongodb.net/?retryWrites=true&w=majority"` und ändert die zwei nächsten Zeile ab zu `client = motor.motor_asyncio.AsyncIOMotorClient(MONGO_DETAILS)` sowie `driftapi_database = client.driftapi` . Nun das Projekt neu bauen und gleich mit einem Online Spiel testen.