...
def app():
    ...
    st.subheader("Create new Time Race Game in Lobby " + str(lobby_id))
    ...
    with placeholder1.container():
        with st.form("my_form", clear_on_submit=True):
            gameOptions = {}
            game_id = st.text_input("Game ID", value="Time1", max_chars=None, key=None, type="default", help=None, autocomplete=None, on_change=None, disabled=False)
            
            time_limit = st.number_input("time [m]", min_value=1, max_value=10000, value=10, step=1, format=None, key=None, help="time limit in minutes", on_change=None, disabled = False)
