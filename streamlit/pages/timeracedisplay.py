...
def app():   
    ...
    st.subheader("Time Race " + str(game_id) + " in Lobby " + str(lobby_id) + " over " + str(int(game["time_limit"])) + " minute(s)")
    ...
    with placeholder0.container():  
        ...
        with col5:
            if st.button(f"Download Data {st.session_state.download_emoji}"):
                st.session_state.nextpage = "download_timerace"
                st.session_state.game_track_images_set = game_track_images_set
                st.session_state.game_track_images = game_track_images
                next_race.empty()
                placeholder0.empty()
            ...
            def constructEntry(r:dict):
                ...
                if "user_name" in r:
                    if ( ( "end_data" in r ) and not ( r["end_data"] is None ) ):
                        ...
                        if(r["laps_completed"] == r["enter_data"]["lap_count"]): # this should not happen in time race
