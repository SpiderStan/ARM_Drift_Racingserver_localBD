...
def app():
    ...
    # make sure that start index is current time + 2 minutes for quick start possibility
                ...
                with st.container():
                    columnLeft, columnRight = st.columns(2)
                    with columnLeft:
                        time_limit_enabled = st.checkbox("Sync. time limit", value=False, key=None, help=None, on_change=None)
                    with columnRight:
                ...
                with st.container():
                    columnLeft, columnRight = st.columns(2)
                    with columnLeft:
                        lap_limit_enabled = st.checkbox("Sync. lap limit", value=False, key=None, help=None, on_change=None)
                    with columnRight:
                        lap_count = st.number_input("laps", min_value=0, max_value=100, value=3, step=1, format=None, key=None, help="number of laps", on_change=None, disabled = False)
                ...
                with st.container():
                    columnLeft, columnRight = st.columns(2)
                    with columnLeft:
                        num_sectors_enabled = st.checkbox("Set Number of Sectors", value=False, key=None, help="if set, you can specify the number of track sections (for the sector display)", on_change=None)
                    with columnRight:
